...
Sympathy Form is a value container with built-in whitelist validation ("accept known good"). It uses language independent validation rules (plain array) that can be reused for client-side validation (JavaScript) or passed to template rendering engines such as Twig or Smarty (HTML). Sympathy Form can be used with any framework (controller and model independent).
